...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        int[][] student = new int[M+1][M+1];
        for(int n=0; n<N; n++){
            student[sc.nextInt()][sc.nextInt()]++;
        }

        int[][] sum = new int[M+1][M+1];
        //sum[h][w]: #students s.t. height <= h && weight <= w
        for(int h=0; h<=M; h++)for(int w=0; w<=M; w++){
            sum[h][w] = (w>0 ? sum[h][w-1] : 0) + (h>0 ? sum[h-1][w] : 0) - (h>0&&w>0 ? sum[h-1][w-1] : 0) + student[h][w];
        }
 
        int ans = 0;
        for(int h=K; h<=M; h++)for(int w=K; w<=M; w++){
            ans = Math.max(ans, sum[h][w] - (h>K ? sum[h-K-1][w] : 0) - (w>K ? sum[h][w-K-1] : 0) + (h>K&&w>K ? sum[h-K-1][w-K-1] : 0));
        }
