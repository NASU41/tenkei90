...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int n=0; n<N; n++) for(int s=0; s<=S; s++) dp[n][s] = -1;
        if(A[0] <= S) dp[0][A[0]] = 0;
        if(B[0] <= S) dp[0][B[0]] = 1;
        for(int n=1; n<N; n++) for(int s=0; s<=S; s++){
            if(s-A[n] >= 0 && dp[n-1][s-A[n]] >= 0) dp[n][s] = 0;
            if(s-B[n] >= 0 && dp[n-1][s-B[n]] >= 0) dp[n][s] = 1;            
        }

        if(dp[N-1][S]==-1) pr.println("Impossible");
        else{
