...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        double X = sc.nextDouble(), Y = sc.nextDouble(), Z = 0;

        int Q = sc.nextInt();
        for(int q=0; q<Q; q++){
            double E = sc.nextDouble();
            double x = 0, y = - (L/2) * Math.sin(2*Math.PI * E/T), z = L/2 - (L/2) * Math.cos(2*Math.PI * E/T);
            Vector sight = new Vector(X-x, Y-y, Z-z);
            Vector horizon = new Vector(X-x, Y-y, 0);
            pr.println(Math.toDegrees(sight.angle(horizon)));
        }
...
class Vector{
    ...
    public Vector cross(Vector another){
        return new Vector(this.y*another.z - this.z*another.y,
    ...
    public double angle(Vector another){
        return Math.asin(this.cross(another).length()/(this.length() * another.length()));
    }
