...
class Query{
    ...
    public Query(int t, int x, int y, long v){
        this.type = t;
        this.x = x;
        this.y = y;
        this.v = v;
...
public class Main {
    public static void main(String[] args){
        ...
        for(int q=0; q<Q; q++){
            query[q] = new Query(sc.nextInt(), sc.nextInt()-1, sc.nextInt()-1, sc.nextLong());
        }

        long[] s = new long[N-1];
        for(int q=0; q<Q; q++) if(query[q].type == 0){
            s[query[q].x] = query[q].v;
        }
        long[] a = new long[N];
        for(int n=1; n<N; n++) a[n] = s[n-1] - a[n-1];
        System.err.println(Arrays.toString(a));
        DSU uf = new DSU(N);
        for(int q=0; q<Q; q++){
            if(query[q].type==0){
                uf.merge(query[q].x, query[q].y);
            }else{
                if(uf.same(query[q].x, query[q].y)){
                    if((query[q].y-query[q].x)%2==0){
                        pr.println(a[query[q].y] + query[q].v - a[query[q].x]);
                    }else{
                        pr.println(a[query[q].y] - query[q].v + a[query[q].x]);
                    }
                }else{
