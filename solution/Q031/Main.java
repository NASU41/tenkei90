...
public class Main {
    private static int mex(Set<Integer> set){
        for(int n=0; n<set.size()+10; n++){
            if(!set.contains(n)) return n;
        }
        return -1;
    }
    private static int[][] grundy(){
        int[][] ans = new int[60][2000];
        for(int W=0; W<=50; W++)for(int B=0; B<1700; B++){
            Set<Integer> next = new HashSet<>();
            if(W >= 1) next.add(ans[W-1][B+W]);
            for(int k=1; k<=B/2; k++) next.add(ans[W][B-k]);
            ans[W][B] = mex(next);
        }
        return ans;
    }
    public static void main(String[] args) throws Exception{
        ...
        final int[] W = sc.nextIntArray(N), B = sc.nextIntArray(N);
