...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        ModIntFactory fact = new ModIntFactory(1_000_000_007);
        ...
        int[] pow10 = new int[100];
        pow10[0] = 10%B;
        for(int i=1; i<100; i++) pow10[i] = (pow10[i-1]*pow10[i-1]) % B;
        
        ModIntFactory.ModInt[][] dp = new ModIntFactory.ModInt[62][B];
        for(int i=0; i<62; i++)for(int b=0; b<B; b++) dp[i][b] = fact.create(0);
        for(int c:C) dp[0][c%B].addAsg(1);
        for(int i=0; i<62-1; i++){
            for(int j=0; j<B; j++)for(int k=0; k<B; k++){
                dp[i+1][(pow10[i]*j+k)%B].addAsg(dp[i][j].mul(dp[i][k]));
            }
        }        

        ModIntFactory.ModInt[] cur = new ModIntFactory.ModInt[B];
        for(int b=1; b<B; b++) cur[b] = fact.create(0);
        cur[0] = fact.create(1);

        for(int i=0; i<62; i++) if(get(N, i)){
            ModIntFactory.ModInt[] next = new ModIntFactory.ModInt[B];
            for(int b=0; b<B; b++) next[b] = fact.create(0);
            for(int j=0; j<B; j++)for(int k=0; k<B; k++){
                next[(pow10[i]*j+k)%B].addAsg(cur[j].mul(dp[i][k]));
            }
            cur = next;
        }
