...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        int N = sc.nextInt();
        Pair<Long,Long>[] point = new Pair[N];
        for(int n=0; n<N; n++) point[n] = new Pair<>(sc.nextLong(), sc.nextLong());

        double ans = 0; // in radian

        for(int j=0; j<N; j++){
            TreeSet<Double> set = new TreeSet<>();
            for(int i=0; i<N; i++)if(i!=j){
                double arg = Math.atan2(point[i].getSecond() - point[j].getSecond(), point[i].getFirst() - point[j].getFirst());
                set.add(arg);
            }
            for(double a: set){
                if(set.floor(a - Math.PI) != null){
                    double arg = a - set.floor(a - Math.PI);
                    ans = Math.max(ans, Math.min(arg, 2*Math.PI-arg));
                } 
                if(set.ceiling(a - Math.PI) != null){
                    double arg = a - set.ceiling(a - Math.PI);
                    ans = Math.max(ans, Math.min(arg, 2*Math.PI-arg));
                }
                if(set.floor(a + Math.PI) != null){
                    double arg = a - set.floor(a + Math.PI);
