...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        final ModIntFactory fact = new ModIntFactory(1_000_000_007);

        int N = sc.nextInt();
        ModIntFactory.ModInt[] ans = new ModIntFactory.ModInt[N+1];
        for(int k=0; k<=N; k++) ans[k] = fact.create(0);

        for(int k=1; k<=N; k++){
            for(int balls = 1; (balls-1)*k+1 <= N; balls++){
                ans[k].addAsg(fact.combination(N - (k-1)*(balls-1), balls));
            }
        }
