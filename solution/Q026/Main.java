...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int e=0; e<N-1; e++){
            tree.addEdge(sc.nextInt()-1, sc.nextInt()-1);
        }

        long[] distance = tree.bfs01(0);

        List<Integer> even = new ArrayList<>(), odd = new ArrayList<>();
        for(int n=0; n<N; n++){
            if(distance[n]%2==0) even.add(n);
            else odd.add(n);
        }

        long[] ans = new long[N/2];
        if(even.size() >= N/2){
            for(int n=0; n<N/2; n++) ans[n] = even.get(n);
        }else{
            for(int n=0; n<N/2; n++) ans[n] = odd.get(n);
        }

        pr.printArray(ans, x -> x+1);
