...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        int N = sc.nextInt();
        String S = sc.next();
        ModIntFactory.ModInt[][] dp = new ModIntFactory.ModInt[N][7+1];
        for(int n=0; n<N; n++)for(int i=0; i<=7; i++) dp[n][i] = fact.create(0);
        dp[0][0] = fact.create(1);
        if(S.charAt(0)=='a') dp[0][1].addAsg(1);
        for(int n=1; n<N; n++) for(int i=0; i<=7; i++){
            dp[n][i].addAsg(dp[n-1][i]);
            if(i>0 && S.charAt(n)=="atcoder".charAt(i-1)) dp[n][i].addAsg(dp[n-1][i-1]);
        }
        pr.println(dp[N-1][7]);
        pr.close();
    }
