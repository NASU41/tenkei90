...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        int N = sc.nextInt();
        Pair<Long,Long>[] factory = new Pair[N];
        for(int n=0; n<N; n++) factory[n] = new Pair<>(sc.nextLong(), sc.nextLong());

        long[] X = new long[N], Y = new long[N];
        for(int n=0; n<N; n++){
            X[n] = factory[n].getFirst();
            Y[n] = factory[n].getSecond();
        }
        
        Pair<Long,Long> plant = new Pair<>(median(X), median(Y));

        long ans = 0;
        for(Pair<Long,Long> f:factory){
            ans += Math.abs(f.getFirst()-plant.getFirst()) + Math.abs(f.getSecond()-plant.getSecond());
        }
