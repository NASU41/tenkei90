...
class SumBoundedArrayIterator implements Iterator<int[]>, Iterable<int[]>{
    int length;
    int maxSum;
...
public class Main {
    private static int[] digitAppearance(long n){
        String s = Long.toString(n);
        int[] a = new int[10];
        for(int i=0; i<s.length(); i++) a[s.charAt(i)-'0']++;
        return a;
    }
    private static boolean match(long n, int[] a){
        int[] b = digitAppearance(n);
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int[] array: new SumBoundedArrayIterator(10, 12)){
            long f = 1;
            for(int i=0; i<10; i++) for(int j=0; j<array[i]; j++) f *= i;
            long m = f + B;
            if(0<m && m<=N && match(m, array)){
                ans++;
            } 
        }
