...
public class Main {
    private static List<Pair<Character,Integer>> runLength(String S){
        int N = S.length();
        ...
        for(int n=0; n<N; n++){
            if(S.charAt(n)==c){
                ...
                ans.add(new Pair<>(c, len));
                c = S.charAt(n);
                len = 1;
            }
        }
        if(len > 0) ans.add(new Pair<>(c,len));
        return ans;
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        List<Pair<Character,Integer>> rle = runLength(S);

        long ans = 1L * N * (N-1) / 2;
        for(Pair<Character,Integer> p: rle){
            long l = p.getSecond();
            ans -= l * (l-1) / 2;
        }
