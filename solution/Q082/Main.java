...
public class Main {
    static final ModIntFactory fact = new ModIntFactory(1_000_000_007);
    ...
    private static ModIntFactory.ModInt solve(long N){
        
        ModIntFactory.ModInt ans = fact.create(0);
        for(int length = 1; length<=18; length++){
            long min = pow(10, length-1);
            long max = Math.min(pow(10, length) - 1, N);

            if(N < min) break;

            ModIntFactory.ModInt count = fact.create(min+max).mulAsg((max-min+1)).divAsg(2);
            
            ans.addAsg(count.mul(length));
        }
        if(N==1000000000000000000L){
            ans.addAsg(fact.create(1000000000000000000L).mulAsg(19));
        }
        return ans;
    }
    public static void main(String[] args) throws Exception{
        ...
        pr.println(solve(R).sub(solve(L-1)));
