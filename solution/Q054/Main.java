...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        for(int m=0; m<M; m++){
            int K = sc.nextInt();
            int[] R = sc.nextIntArray(K, x -> x-1);
            for(int r: R) graph.addEdge(r, N+m);
        }

        long[] distance = graph.bfs01(0);
        for(int n=0; n<N; n++){
            if(distance[n] > 1_000_000_000L) pr.println(-1);
            else pr.println(distance[n]/2);
        }
        pr.close();
    }
}
