...
public class Main {
    public static void main(String[] args){
        ...
        int N = sc.nextInt();
        long[] A = sc.nextLongArray(N), B = sc.nextLongArray(N), C = sc.nextLongArray(N);

        Multiset<Long> x = new Multiset<>(), y = new Multiset<>(), z = new Multiset<>();
        for(long a:A) x.addOne(a%46);
        for(long b:B) y.addOne(b%46);
        for(long c:C) z.addOne(c%46);

        long ans = 0;

        for(long i=0; i<46; i++)for(long j=0; j<46; j++){
            long k = (46*100-i-j)%46;
            ans += x.count(i)*y.count(j)*z.count(k);
        }
