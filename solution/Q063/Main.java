...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        int[][] P = sc.nextIntMatrix(H, W);
 
        int[][] part = new int[1<<H][W];
        for(int w=0; w<W; w++){
            for(int pattern=1; pattern<(1<<H); pattern++){
                if(Integer.bitCount(pattern)==1){
                    part[pattern][w] = P[Integer.numberOfTrailingZeros(pattern)][w];
                } else if(part[pattern ^ Integer.lowestOneBit(pattern)][w]==P[Integer.numberOfTrailingZeros(pattern)][w]){
                    part[pattern][w] = P[Integer.numberOfTrailingZeros(pattern)][w];
                } else{
                    part[pattern][w] = -1;
                }
        ...
        for(int pattern=1; pattern<(1<<H); pattern++){
            ...
            for(int w=0; w<W; w++) counter.addOne(part[pattern][w]);
            for(int p:counter.keySet()) if(p>=0){
                ans = Math.max(ans, Integer.bitCount(pattern)*counter.get(p));
            }
        }
