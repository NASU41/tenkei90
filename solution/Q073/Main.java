...
public class Main {
    private static long[] red, blue, both;
    ...
    private static void solve(char[] c, UndirectedGraph graph, int start){
        int N = graph.V;
        red = new long[N]; blue = new long[N]; both = new long[N];
        solve(c, graph, new HashSet<>(), start);
    }    
    private static void solve(char[] c, UndirectedGraph graph, Set<Integer> visited, int cur){
        visited.add(cur);
        if(c[cur]=='a'){
            red[cur] = 1;
            both[cur] = 1;
            for(UndirectedGraph.Edge e: graph.getEdges(cur)){
                int v = e.getOtherVertex(cur);
                if(!visited.contains(v)){
                    solve(c,graph, visited, v);
                    red[cur] = safe_mod(red[cur] * (red[v]+both[v]), MOD);
                    both[cur] = safe_mod(both[cur] * (red[v]+blue[v]+2*both[v]), MOD);
                }
            }
            both[cur] = safe_mod(both[cur] - red[cur], MOD);
        }else{
            blue[cur] = 1;
            both[cur] = 1;
            for(UndirectedGraph.Edge e: graph.getEdges(cur)){
                ...
                if(!visited.contains(v)){
                    solve(c,graph, visited, v);
                    blue[cur] = safe_mod(blue[cur] * (blue[v]+both[v]), MOD);
                    both[cur] = safe_mod(both[cur] * (red[v]+blue[v]+2*both[v]), MOD);
                }
            }
            both[cur] = safe_mod(both[cur] - blue[cur], MOD);
        }
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        char[] c = new char[N];
        for(int n=0; n<N; n++) c[n] = sc.next().charAt(0);
