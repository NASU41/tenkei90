...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int n=0; n<N; n++){
            A[n] = new Pair<>(sc.nextLong(), sc.nextLong());
        }
        Pair<Long,Long>[] B = new Pair[N];
        for(int n=0; n<N; n++){
            B[n] = new Pair<>(sc.nextLong(), sc.nextLong());
        }
        
        Map<Pair<Long,Long>, Integer> data = new HashMap<>();
        for(int n=0; n<N; n++) data.put(B[n], n);
        
        MaxFlow flow = new MaxFlow(N+N+2);
        // airplane[n] -> n
        // destination[n] -> N+n
        final int s = N*2, t = N*2+1;

        for(int n=0; n<N; n++){
            flow.addEdge(s, n, 1);
            flow.addEdge(N+n, t, 1);
            for(int d=0; d<8; d++){
                Pair<Long,Long> destination = new Pair<>(A[n].getFirst()+dx[d]*T, A[n].getSecond()+dy[d]*T);
                if(data.containsKey(destination)) flow.addEdge(n, N+data.get(destination), 1);
            }
        }

        

        long match = flow.maxFlow(s, t);
        if(match < N){
            pr.println("No");
        }else{
            pr.println("Yes");
            ...
            for(var e: flow.getEdges()){
                ...
                Pair<Long,Long> airplane = A[e.from];
                Pair<Long,Long> destination = B[e.to - N];
                for(int d=0; d<8; d++){                    
                    if(new Pair<>(airplane.getFirst()+dx[d]*T, airplane.getSecond()+dy[d]*T).equals(destination)){
                        ans[e.from] = d+1;
                    }
                }
            }
