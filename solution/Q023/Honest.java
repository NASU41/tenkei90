...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        dp[0][0].put(0, 1L);
        if(board[0][0] != '#') dp[0][0].put(1<<W, 1L);
        ...
        for(int h=0; h<H; h++)for(int w=0; w<W; w++){
            ...
            for(int p:dp[h][w].keySet()){
                ...
                dp[nextH][nextW].put(p>>1, (dp[nextH][nextW].getOrDefault(p>>1, 0L) + dp[h][w].getOrDefault(p, 0L))%MOD);

                // place a king on board[nextH][nextW]
                boolean attacked = false;
                attacked |= onBoard(H,W, nextH, nextW-1) && get(p, W);
                attacked |= onBoard(H,W, nextH-1, nextW+1) && get(p,2);
                attacked |= onBoard(H,W, nextH-1, nextW) && get(p, 1);
                attacked |= onBoard(H,W, nextH-1, nextW-1) && get(p,0);
                attacked |= board[nextH][nextW]=='#';
                if(!attacked){
                    dp[nextH][nextW].put((p>>1) | (1<<W), (dp[nextH][nextW].getOrDefault((p>>1) | (1<<W), 0L) + dp[h][w].getOrDefault(p, 0L))%MOD);
                }                
            }
