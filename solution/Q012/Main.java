    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int q=0; q<Q; q++){
            ...
            if(t==1){
                ...
                board[r][c] = true;
                if(r>0   && board[r-1][c]) uf.merge(r*W+c, (r-1)*W+c    );
                if(r<H-1 && board[r+1][c]) uf.merge(r*W+c, (r+1)*W+c    );
                if(c>0   && board[r][c-1]) uf.merge(r*W+c, r    *W+(c-1));
                if(c<W-1 && board[r][c+1]) uf.merge(r*W+c, r    *W+(c+1));                
            }else{
                int ra = sc.nextInt()-1, ca = sc.nextInt()-1;
                int rb = sc.nextInt()-1, cb = sc.nextInt()-1;
                if(board[ra][ca] && board[rb][cb] && uf.same(ra*W+ca, rb*W+cb)) pr.println("Yes");
                else pr.println("No");
            }
        }
