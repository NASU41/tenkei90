...
public class Main {
    ...
    private static int[] subtreeSize(UndirectedGraph graph, int root){
        int[] ans = new int[graph.V];
        Arrays.fill(ans, -2);
        subtreeSize(graph, root, ans);
        return ans;
    }
    private static int subtreeSize(UndirectedGraph graph, int cur, int[] ans){
        if(ans[cur] >= 0) return ans[cur];
        ans[cur] = -1;
        int a = 1;
        for(UndirectedGraph.Edge e: graph.getEdges(cur)){
            int v = e.getOtherVertex(cur);
            if(ans[v] == -2) a += subtreeSize(graph, v, ans);
        }
        ans[cur] = a;
        return a;
    }
    public static void main(String[] args){
        ...
        int[] sub = subtreeSize(G, 0);
        //pr.println(Arrays.toString(sub));

        long ans = 0;
        for(int v=1; v<N; v++){
            ans += 1L * sub[v] * (N - sub[v]);
        }
        pr.println(ans);
