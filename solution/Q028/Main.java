...
public class Main {
    public static void main(String[] args) throws Exception{
        ...
        for(int n=0; n<N; n++){
            ...
            c[lx][ly]++; c[lx][ry]--; c[rx][ly]--; c[rx][ry]++;
        }

        int[][] p = new int[L+1][L+1];
        for(int i=0; i<=L; i++)for(int j=0; j<=L; j++){
            p[i][j] = (i>0 ? p[i-1][j] : 0) + (j>0 ? p[i][j-1] : 0) - (i>0&&j>0 ? p[i-1][j-1] : 0) + c[i][j];
        }

        long[] count = new long[N+1];
        for(int i=0; i<=L; i++)for(int j=0; j<=L; j++) count[p[i][j]]++;

        for(int k=1; k<=N; k++) pr.println(count[k]);
